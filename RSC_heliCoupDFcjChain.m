function [uu,uusym] = RSC_heliCoupDFcjChain(varargin) %#ok<*AGROW>
%% Chain any D: pp1, sX1, pp2, sX2, ...
%% uu & uusym per 6+1 col for tt & ttsym (DF)
lv=length(varargin);
if (fix(lv/2)~=lv/2)
...
ts=zeros(1,lv/2);
for ii=1:1:(lv/2)
  [temp{ii},tempsym{ii}]=RSC_heliCoupDFcj(varargin{2*ii-1},varargin{2*ii});
  [ts(ii),~]=size(temp{ii});
end
tsm=max(ts);
for ii=1:1:(lv/2)
